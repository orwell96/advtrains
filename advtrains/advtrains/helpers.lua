...
function advtrains.dumppath(path)
	minetest.chat_send_all("Dumping a path:")
	if not path then minetest.chat_send_all("dumppath: no path(nil)") return end
	local temp_path={}
	...
	for _,pit in ipairs(temp_path) do
		minetest.chat_send_all(pit.i.." > "..minetest.pos_to_string(pit.p))
	end
end
